		...
		for step_index = match_settings.constants.SIMULATION_OFFSET, simulation_depth do
			...
			for _, collision in ipairs(collisions) do
				...
				if alive_1 and alive_2 then
					...
					local attack_1 = player_1.in_attack
					...
					local attack_2 = player_2.in_attack
					local killer_2 = killers[client_2]
					local distance_2 = collision.distance_2 >= collision.distance_1 + simulation_step

					-- Player 1 attacked, but player 2 was not in attack -> Player 1 is winner
					local winner_1_type_1 = movement_1 and attack_1 and not killer_1 and not attack_2
					-- Player 1 attacked, but player 2 already killed somebody else -> Player 1 is winner
					local winner_1_type_2 = movement_1 and attack_1 and not killer_1 and killer_2
					-- Player 1 attacked while moved longer path than player 2 -> Player 1 is winner
					local winner_1_type_3 = movement_1 and attack_1 and not killer_1 and distance_1

					local winner_2_type_1 = movement_2 and attack_2 and not killer_2 and not attack_1
					local winner_2_type_2 = movement_2 and attack_2 and not killer_2 and killer_1
					local winner_2_type_3 = movement_2 and attack_2 and not killer_2 and distance_2

					local mutual_kill_type_1 = movement_1 and movement_2 and attack_1 and attack_2 and not killer_1 and not killer_2
					...
					if winner_1_type_1 or winner_1_type_2 or winner_1_type_3 then
						...
						local match_event = match_events:create_player_killed_event(client_2, client_1)
					...
					elseif winner_2_type_1 or winner_2_type_2 or winner_2_type_3 then
						...
						local match_event = match_events:create_player_killed_event(client_1, client_2)
						events_container:add_event(match_event)
					...
					elseif mutual_kill_type_1 then
						...
						player_1:kill_player()
						player_2:kill_player()
						local match_event_1 = match_events:create_player_killed_event(client_1, client_2)
						events_container:add_event(match_event_1)
						local match_event_2 = match_events:create_player_killed_event(client_2, client_1)
