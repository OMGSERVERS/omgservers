...
class AdminGenerateIdOperationImpl implements AdminGenerateIdOperation {
    ...
    public void execute(final String service, final String user) {
        ...
        final var generatedId = adminClient.execute(request)
                .map(GenerateIdAdminResponse::getId)
                .await().indefinitely();

        appendResultMapOperation.execute(path, KeyEnum.ID, String.valueOf(generatedId));
    }
}
