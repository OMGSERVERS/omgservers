...
class DeveloperLocalTestingDeployVersionOperationImpl implements DeveloperLocalTestingDeployVersionOperation {
    ...
    public void execute(final String tenant,
        ...
        final var developer = testTenantLog.getDeveloper();
        final var password = testTenantLog.getPassword();

        final var developerClient = createLocalTestingDeveloperClientOperation.execute(developer, password);

        final var versionId = createVersion(developerClient, tenant, project, config);

        final var dockerDaemonUri = LocalTestingConfiguration.DOCKER_DAEMON_URI;
        final var registryUri = LocalTestingConfiguration.REGISTRY_URI;
