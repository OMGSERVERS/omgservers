package com.omgservers.service.module.runtime.operation.testInterface;

import com.omgservers.model.runtimeClient.RuntimeClientModel;
import com.omgservers.service.module.runtime.impl.operation.upsertRuntimeClient.UpsertRuntimeClientOperation;
import com.omgservers.service.operation.changeWithContext.ChangeContext;
import io.smallrye.mutiny.Uni;
import io.vertx.mutiny.pgclient.PgPool;
import jakarta.enterprise.context.ApplicationScoped;
import lombok.AllArgsConstructor;
import lombok.extern.slf4j.Slf4j;

import java.time.Duration;

@Slf4j
@ApplicationScoped
@AllArgsConstructor
public class UpsertRuntimeClientOperationTestInterface {
    private static final long TIMEOUT = 1L;

    final UpsertRuntimeClientOperation upsertRuntimeClientOperation;

    final PgPool pgPool;

    public ChangeContext<Boolean> upsertRuntimeClient(final int shard,
                                                      final RuntimeClientModel runtimeClient) {
        return Uni.createFrom().context(context -> {
                    final var changeContext = new ChangeContext<Boolean>(context);
                    return pgPool.withTransaction(sqlConnection -> upsertRuntimeClientOperation
                                    .upsertRuntimeClient(changeContext, sqlConnection, shard, runtimeClient))
                            .invoke(changeContext::setResult)
                            .replaceWith(changeContext);
                })
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }
}
