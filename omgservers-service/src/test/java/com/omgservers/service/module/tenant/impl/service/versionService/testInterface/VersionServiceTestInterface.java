...
public class VersionServiceTestInterface {
    ...
    public SelectStageVersionResponse selectStageVersion(final SelectStageVersionRequest request) {
        return versionService.selectStageVersion(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public GetVersionLobbyRequestResponse getVersionLobbyRequest(final GetVersionLobbyRequestRequest request) {
        return versionService.getVersionLobbyRequest(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public FindVersionLobbyRequestResponse findVersionLobbyRequest(final FindVersionLobbyRequestRequest request) {
        return versionService.findVersionLobbyRequest(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public ViewVersionLobbyRequestsResponse viewVersionLobbyRequests(final ViewVersionLobbyRequestsRequest request) {
        return versionService.viewVersionLobbyRequests(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public SyncVersionLobbyRequestResponse syncVersionLobbyRequest(final SyncVersionLobbyRequestRequest request) {
        return versionService.syncVersionLobbyRequest(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public DeleteVersionLobbyRequestResponse deleteVersionLobbyRequest(final DeleteVersionLobbyRequestRequest request) {
        return versionService.deleteVersionLobbyRequest(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public GetVersionLobbyRefResponse getVersionLobbyRef(final GetVersionLobbyRefRequest request) {
        return versionService.getVersionLobbyRef(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public FindVersionLobbyRefResponse findVersionLobbyRef(final FindVersionLobbyRefRequest request) {
        return versionService.findVersionLobbyRef(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public ViewVersionLobbyRefsResponse viewVersionLobbyRefs(final ViewVersionLobbyRefsRequest request) {
        return versionService.viewVersionLobbyRefs(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public SyncVersionLobbyRefResponse syncVersionLobbyRef(final SyncVersionLobbyRefRequest request) {
        return versionService.syncVersionLobbyRef(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public DeleteVersionLobbyRefResponse deleteVersionLobbyRef(final DeleteVersionLobbyRefRequest request) {
    ...
    public GetVersionMatchmakerRefResponse getVersionMatchmakerRef(final GetVersionMatchmakerRefRequest request) {
        return versionService.getVersionMatchmakerRef(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public FindVersionMatchmakerRefResponse findVersionMatchmakerRef(final FindVersionMatchmakerRefRequest request) {
        return versionService.findVersionMatchmakerRef(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public ViewVersionMatchmakerRefsResponse viewVersionMatchmakerRefs(final ViewVersionMatchmakerRefsRequest request) {
        return versionService.viewVersionMatchmakerRefs(request)
                .await().atMost(Duration.ofSeconds(TIMEOUT));
    }

    public SyncVersionMatchmakerRefResponse syncVersionMatchmakerRef(final SyncVersionMatchmakerRefRequest request) {
