...
@Slf4j
@ApplicationScoped
class GetAliasModuleClientOperationImpl implements GetAliasModuleClientOperation {

    final Map<URI, AliasModuleClient> cache;

    GetAliasModuleClientOperationImpl() {
        cache = new ConcurrentHashMap<>();
    }

    @Override
    public synchronized AliasModuleClient getClient(final URI uri) {
        if (!cache.containsKey(uri)) {
            final var client = RestClientBuilder.newBuilder()
                    .baseUri(uri)
                    .build(AliasModuleClient.class);
