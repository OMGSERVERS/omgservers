...
@Slf4j
@ApplicationScoped
class GetMatchModuleClientOperationImpl implements GetMatchModuleClientOperation {

    final Map<URI, MatchModuleClient> cache;

    GetMatchModuleClientOperationImpl() {
        cache = new ConcurrentHashMap<>();
    }

    @Override
    public synchronized MatchModuleClient getClient(final URI uri) {
        if (!cache.containsKey(uri)) {
            final var client = RestClientBuilder.newBuilder()
                    .baseUri(uri)
                    .build(MatchModuleClient.class);
