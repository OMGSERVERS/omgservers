...
class GetServiceClientOperationImpl implements GetServiceClientOperation {
    ...
    final Map<URI, ServiceModuleClient> cache;
    ...
    @Override
    public synchronized ServiceModuleClient getClient(final URI uri) {
        if (!cache.containsKey(uri)) {
            final var client = RestClientBuilder.newBuilder()
                    .baseUri(uri)
                    .build(ServiceModuleClient.class);

            log.debug("Service client was created, uri={}", uri);
